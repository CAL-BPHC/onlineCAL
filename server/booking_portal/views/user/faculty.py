...
def faculty_usage_summary(request):
    ...
    return usage_summary(request, reporting.collect_usage, request.user)
    ...
    try:
        with transaction.atomic():
            ...
            if not faculty.department:
                error = (
                    "You need to be assigned to a department to accept requests. "
                    "Please contact the admin."
                )
                messages.error(request, error)
                return redirect("instrument-list")
            # Every request is billed to the department, whose approval is
            # taken as given: it goes straight on to the lab assistant.
